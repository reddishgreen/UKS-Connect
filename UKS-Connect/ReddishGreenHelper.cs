...
namespace ReddishGreen_CRM_Helper
{
    ...
    public class ReddishGreenHelper
    {
        ...
        public List<Entity> GetManyToManyEntity2(
    Guid stockId,
    string relatedEntity,
    string manyToManyRelationshipTable,
    string stockIdOneField,
    string stockIdTwoField)
        {
            // Create the query for the related entity
            var query = new QueryExpression(relatedEntity)
            {
                ColumnSet = new ColumnSet(true) // Retrieve all columns; adjust to only include necessary fields
            };

            // Add the link to the many-to-many relationship table
            var link = new LinkEntity
            {
                LinkFromEntityName = relatedEntity,
                LinkFromAttributeName = "rg_stockid", // Primary key of rg_stock
                LinkToEntityName = manyToManyRelationshipTable,
                LinkToAttributeName = stockIdTwoField, // Linking rg_stock to rg_stockidtwo
                JoinOperator = JoinOperator.Inner,
                LinkCriteria = new FilterExpression
                {
                    FilterOperator = LogicalOperator.Or, // Include relationships in both directions
                    Conditions =
            {
                new ConditionExpression(stockIdOneField, ConditionOperator.Equal, stockId),
                new ConditionExpression(stockIdTwoField, ConditionOperator.Equal, stockId)
            }
                }
            };
